...
useEffect(() => {
  ...
  // Leave balance validation (employees only)
  if (
    user?.role !== ROLES.ADMIN &&
    leaveDays > employee?.leaveBalance
  ) {
    setError(
      `Insufficient leave balance. You have ${employee.leaveBalance} days available.`
    )
    return false
  }
...
if (!employee && user?.role !== ROLES.ADMIN) {
  ...
  setFormData({
    ...formData,
    reason: e.target.value.slice(0, 250)
  })
}
                        required
                        disabled={submitLoading}
