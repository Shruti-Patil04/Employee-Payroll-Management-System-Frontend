...
const isAdmin = user?.role === ROLES.ADMIN
  ...
  const loadEmployeeData = async () => {
    try {
      ...
      const [leaveData, payrollData, summaryResponse] =
  await Promise.all([
    leaveService.getLeaveRequestsByEmployee(id)
      .catch(() => []),
...
// Fill summaryResponse with counts
summaryResponse.CASUAL = leaveData.filter(
  leave => leave.leaveType === 'CASUAL'
).length

summaryResponse.SICK = leaveData.filter(
  leave => leave.leaveType === 'SICK'
).length

summaryResponse.PAID = leaveData.filter(
  leave => leave.leaveType === 'PAID'
).length

summaryResponse.UNPAID = leaveData.filter(
  ...
  <div className="card-body">
    <div className="d-flex justify-content-between mb-2">
      ...
      <strong>{leaveSummary.CASUAL || 0}</strong>
    </div>

    <div className="d-flex justify-content-between mb-2">
      <span>Sick Leave</span>
      <strong>{leaveSummary.SICK || 0}</strong>
    </div>

    <div className="d-flex justify-content-between mb-2">
      <span>Paid Leave</span>
      <strong>{leaveSummary.PAID || 0}</strong>
    </div>

    <div className="d-flex justify-content-between">
      <span>Unpaid Leave</span>
      <strong>{leaveSummary.UNPAID || 0}</strong>
    </div>
  </div>
</div>
        </div>
