...
const JobRoleForm = () => {
  ...
  if (!/^[A-Za-z\s&]+$/.test(formData.jobTitle)) {
    setError('Job title should contain only letters')
    return false
  }

  // Minimum length
  if (formData.jobTitle.trim().length < 3) {
  ...
  if (formData.baseSalary < 1000) {
    setError('Salary should be at least ₹1000')
    return false
  }
  ...
  setFormData({
    ...formData,
    jobTitle: e.target.value.replace(/[^A-Za-z &]/g, '')
  })
}
                    required
  ...
  setFormData({
    ...formData,
    baseSalary: e.target.value.replace(/\D/g, '')
  })
}
                      required
                      disabled={submitLoading}
